...
export const handleJobApply = async (jobId:number,application:FormData) => {
  ...
  export const totalJobApplied = async () =>{
    try {
      const token = getToken();
      const response = await axios.get(`http://localhost:8000/application/totalJobApplied`,
        {
          headers: {
          Authorization: `Bearer ${token}`
        }}
  ...
  export const totalJobRejected = async () =>{
    try {
      const token = getToken();
      const response = await axios.get(`http://localhost:8000/application/totalJobRejected`,
        {
          headers: {
          Authorization: `Bearer ${token}`
        }}
